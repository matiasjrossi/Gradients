    ...
    case HORIZONTAL:
        for (int i=0; i<image.width(); i++)
            for (int j=0; j<image.height(); j++)
                image.setPixel(i, j, gradient->getColor(100*i/image.width()).rgb());
        break;
    case VERTICAL:
        for (int i=0; i<image.width(); i++)
            for (int j=0; j<image.height(); j++)
                image.setPixel(i, j, gradient->getColor(100*(image.height()-j-1)/image.height()).rgb());
        break;
    case DIAGONAL:
        for (int i=0; i<image.width(); i++)
            for (int j=0; j<image.height(); j++)
                image.setPixel(i, j, gradient->getColor(100*(i+j)/(image.height()+image.width())).rgb());
        break;
    }
